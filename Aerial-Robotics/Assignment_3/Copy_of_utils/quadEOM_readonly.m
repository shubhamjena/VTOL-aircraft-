function sdot = quadEOM_readonly( s, u, params)
...
% Expressing T1,T2,alpha1,alpha2 in terms of control input vector u
 ...
 T1 = (params.m*params.g-u(2))/2;
 T2 = (params.m*params.g+u(2))/2;
 ...
 u_clamped(1) = (params.m*params.g/2)*(params.A/(pi*params.R*params.R))*(alpha1_clamped+alpha2_clamped); 
 u_clamped(2) = (params.m*params.g/2)*(params.A/(pi*params.R*params.R))*(alpha1_clamped+alpha2_clamped); 
 u_clamped(3) = T2_clamped - T1_clamped;
...
accel = (1 / params.mass) * (wRb * ...
         [0; 
          ...
          T1_clamped+T2_calmped-F1*sin(alpha1_clamped)-F2*sin(alpha2_clamped)])...
